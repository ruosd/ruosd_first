...
from .chroma_store_service import SearchResult, get_chroma_store_service
...
class MemoryService:
    ...
    async def cleanup_old_memories(self) -> int:
        ...
        # 清理短期记忆（TTL过期）
        self._get_collection_name_for_type(MemoryType.SHORT_TERM)
        # ChromaDB不直接支持按时间删除，这里记录一下需要后续实现
        logger.info("跳过短期记忆自动清理（ChromaDB限制）")

        # 清理长期记忆（超过保留天数）
        self._get_collection_name_for_type(MemoryType.LONG_TERM)
        datetime.now() - timedelta(days=self._long_term_retention_days)
    ...
    async def get_memory_stats(self, memory_type: MemoryType | None = None) -> dict[str, Any]:
        """
        获取记忆统计信息
