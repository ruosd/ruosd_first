...
<script setup>
import { ref } from 'vue'; import { useRouter } from 'vue-router'; import { adminAPI } from '../api/admin.js'
const router = useRouter(); const username = ref(''); const password = ref(''); const loading = ref(false); const error = ref('')
const handleLogin = async () => {
  if (!username.value || !password.value) { error.value = '请输入用户名和密码'; return }
  loading.value = true; error.value = ''
  try { await adminAPI.login(username.value, password.value); router.push('/admin/dashboard') }
